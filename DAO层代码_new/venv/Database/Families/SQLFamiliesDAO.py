...
class SQLFamiliesDAO(FamiliesDAO):
    ...
    def delete_family(self, family_name):
        cursor = self.conn.cursor()
        cursor.execute("DELETE FROM Families WHERE family_name = ?", (family_name,))
        self.conn.commit()

    def update_family(self, family_name, family_updateTime, family_id):
        cursor = self.conn.cursor()
        cursor.execute("UPDATE Families SET family_name = ?, update_time = ? WHERE family_id = ?",
                       (family_name, family_updateTime, family_id))
        self.conn.commit()

    def search_family(self, family_id, family_name):
        cursor = self.conn.cursor()
        cursor.execute("SELECT * FROM Families WHERE family_id = ? OR family_name like concat('%',?,'%')", (family_id, family_name))
        rows = cursor.fetchall()
        families = []
        for row in rows:
            family = Families(row[0], row[1], row[2], row[3], row[4])
    ...
    def count_plants_by_family(self):
        cursor = self.conn.cursor()
        sql = "SELECT Families.family_name, COUNT(Plants.plant_id) AS plant_count FROM Families " \
